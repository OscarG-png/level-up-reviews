...
async function checkFavorite() {
  const response = await fetch(`http://localhost:8000/users/${userData.user.id}/favorites`)
  if (response.ok) {
...
async function checkWishlist() {
  const response = await fetch(`http://localhost:8000/users/${userData.user.id}/wishlist`)
  if (response.ok) {
    ...
    <div className="flex flex-col items-center py-16">
      ...
      <div className="w-full max-w-4xl">
        <Card className="bg-gray-50 shadow-2xl bg-gradient-to-b from-gray-600 to-zinc-900 rounded-xl">
          <div className="p-4">
            <h2 className="text-3xl font-bold tracking-tight text-white mb-4">Game Reviews</h2>
          </div>
          <div className="overflow-auto h-96">
            <div className="space-y-4 overflow-hidden px-4">
              {gameReviews.length > 0 ? (
                gameReviews.map(review => (
                  <div key={review.id} className="p-4 bg-gradient-to-br from-slate-100 to-slate-300 rounded-lg shadow">
                    <div className="flex items-center gap-2 mb-2">
                      <Avatar img={review.profile_picture} alt={review.username} size="md" rounded />
                      <div>
                        <h3 className="text-lg font-semibold">{review.username}</h3>
                        <span className="text-sm text-gray-600">Reviewed on: {review.review_date}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center text-sm text-gray-600 mb-2">
                      <p>{review.content}</p>
                      <span className="font-semibold">Rating: <span style={ratingColor(review.rating)}>{review.rating}</span></span>
                    </div>
                  </div>
                ))
              ) : (
